...
do_variable_assignment(struct command const *cmd, int export_all)
{
  for (size_t i = 0; i < cmd->assignment_count; ++i) {
    ...
    /* TODO Assign */
    vars_set(cmd->assignments[i]->name, cmd->assignments[i]->value);

    /* TODO Export */
    if(export_all != 0){
    vars_export(cmd->assignments[i]->name);
    }
  }
  return 0;
}
...
{
  if (src == dst) 
      {gprintf("[move_fd] src= %d dst= %d", src, dst);
        return dst;}
  /* TODO move src to dst */
  dst = dup2(src, dst);
  gprintf("[runner.c movefd] dst = %d",dst);
  /* TODO close src */
    close(src);
  return dst;
}
...
do_io_redirects(struct command *cmd)
{
  ...
  for (size_t i = 0; i < cmd->io_redir_count; ++i) {
    ...
    if (r->io_op == OP_GREATAND || r->io_op == OP_LESSAND) {
      ...
      if (strcmp(r->filename, "-") == 0) {
        ...
        if (*(r->filename) && !*end /* <--- this is part of the strtol idiom */
          ...
          gprintf("this is where fd is trying to open %d", r->io_number);
          int dst = r->io_number;
          dst = dup2(src, dst);
        } else {
    ...
    file_open:;
      ...
      int src = open(r->filename, flags, 0777);
      gprintf("opened fd(src): %d", src);
      ...
      if (r->io_op == OP_GREAT || r->io_op == OP_GREATAND || r->io_op == OP_CLOBBER || r->io_op == OP_DGREAT){
          dst = STDOUT_FILENO;
          gprintf("set dst to out");

      }

      dst = move_fd(src, dst);
      gprintf("moved src to dst: %d", dst);
    ...
    err: /* TODO Anything that can fail should jump here. No silent errors!!! */
      gprintf("Something went wrong try again");
      //perror(errno);
      warn(0);
      status = -1;
      errno = 0;
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
    ...
    if (is_pl) {
      ...
      /* XXX man 2 pipe */
      //something like this -> make_pipline(pipeline_fds);
      int result = pipe(pipeline_fds);
      if (result == -1){
        goto err;}
    ...
    /* Fork if:
     *   Not a builtin, OR,
     *  TODO:XXX TODO Is a builtin, but isn't a foreground command */
    if (!builtin || (is_bg && builtin)) {
      child_pid = fork();
    ...
    if (child_pid == 0) {
      ...
      if (builtin) {
         ...
         * similar to the stuff under if builtin line 425ish */
     ...
     * that here
     * create or assign gid here*/

    /* XXX initially pipeline_gid is set to -1 (unset) */
    gprintf("pipeline gid before if %d", pipeline_gid);
    if (pipeline_gid == -1) {
       ...
       * SETPGID(3) */
      gprintf("mad it into if statement");
      int result = setpgid(child_pid, 0);
      gprintf("this is the setpgid result: %d", result);
      ...
      pipeline_gid = getpgid(child_pid);
      gprintf("this is the pipeline gid result: %d", pipeline_gid);

      /* TODO Add the new group id to the job list. See jobs.h */
      jobs_add(pipeline_gid);
      jid_t job_id = jobs_get_jid(pipeline_gid);
      gprintf("this is jod id result: %d", job_id);
      

      gprintf(stderr, "job ID[%jd] GID: %d\n",  jobs_get_jid(pipeline_gid), pipeline_gid);
