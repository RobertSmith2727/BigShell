...
builtin_cd(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  chdir(target_dir);
  //gprintf("this is the enviornment %s", getenv(target_dir));
  //const char PWD[3] = "PWD";
  //setenv(PWD, getenv(target_dir) , 1);
  return 0;
}
...
builtin_exit(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  if (cmd->word_count > 1){
         ...
         gprintf("setting params to in builtins.c %d", params.status);
      }  
  bigshell_exit();
  return -1;
}
